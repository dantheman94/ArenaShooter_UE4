...
bool UFireMode::Server_Reliable_CreateImpactManager_Validate() { return true; }
void UFireMode::Server_Reliable_CreateImpactManager_Implementation()
{
	...
	_ImpactEffectManager->AttachToActor(_WeaponParentAttached, FAttachmentTransformRules::SnapToTargetNotIncludingScale);
#if WITH_EDITOR
	///FName folderLoc = FName(TEXT("Runtime Spawned Actors/Abstract/Weapons/" + UKismetSystemLibrary::GetClassDisplayName(UGameplayStatics::GetObjectClass(_ImpactEffectManager))));
	///_ImpactEffectManager->SetFolderPath_Recursively(folderLoc);
#endif
}
...
* @return:	void
*/
bool UFireMode::Server_Reliable_SetMisfired_Validate(bool Misfired) { return true; }
...
* @return:	void
*/
bool UFireMode::Server_Reliable_SetMagazineInWeapon_Validate(bool MagIn) { return true; }
...
*
*/
bool UFireMode::Server_Reliable_SetWeaponAttached_Validate(AWeapon* Weapon) { return true; }
...
*
*/
bool UFireMode::Server_Reliable_SetIsFiring_Validate(bool Firing) { return true; }
void UFireMode::Server_Reliable_SetIsFiring_Implementation(bool Firing) { SetIsFiring(Firing); }
void UFireMode::SetIsFiring(bool Firing)
{
	...
	if (_WeaponParentAttached->GetLocalRole() == ROLE_Authority)
...
*
*/
bool UFireMode::Multicast_Unreliable_PlayThirdPersonContrail_Validate(FHitResult HitResult, FVector StartLocation) { return true; }
void UFireMode::Multicast_Unreliable_PlayThirdPersonContrail_Implementation(FHitResult HitResult, FVector StartLocation)
{
	...
	{
		if (_pContrailParticleSystem != NULL && _WeaponParentAttached->GetLocalRole() < ROLE_Authority)
		{
...
*
*/
bool UFireMode::OwningClient_Unreliable_PlayFirstPersonContrail_Validate(FHitResult HitResult, FVector StartLocation) { return true; }
...
void UFireMode::Fire(FHitResult HitResult, FVector CameraRotationXVector, USkeletalMeshComponent* SkCharWepMeshFirstP, USkeletalMeshComponent* SkCharWepMeshThirdP)
{
			...
			if (_pAmmoPool->GetMagazineAmmo() > 0)
			{
				...
				else
				{
					...
					else // Round ISNT in chamber
					{
						...
						// Until a change in design => when duelwielding a weapon, the chambering is performed instantly & automatically
						if (character->IsDuelWielding())
						{
							Server_Reliable_ChamberRound();
						}
						
						// Play chambering animation (atm its the full reload anim, but starting at the chambering action keyframe)
						else
						{
	...
	if (character != NULL)
	{
		...
		if (!bduelWielding)
		{
			...
			USkeletalMeshComponent* armsMesh = character->_FirstPerson_Arms;
		...
		else // bduelWielding == true
		{
			...
			// Play firing animation on the correct arm mesh
			bool bPrimary = _WeaponParentAttached->IsOwnersPrimaryWeapon();
			USkeletalMeshComponent* armsMesh = bPrimary ? character->_FirstPerson_ArmsDuelRight : character->_FirstPerson_ArmsDuelLeft;			
			if (bPrimary)
				character->OwningClient_PlayPrimaryWeaponFPAnimation(armsMesh, 1.0f, false, true, handByte, 0.0f, true, gunByte, 0.0f);
			else
				character->OwningClient_PlaySecondaryWeaponFPAnimation(armsMesh, 1.0f, false, true, handByte, 0.0f, true, gunByte, 0.0f);
		}
	}
...
*
*/
bool UFireMode::Server_Reliable_FireProjectileTrace_Validate(APawn* Pawn, FHitResult hitResult, FVector CameraRotationXVec, USkeletalMeshComponent* SkCharWepMeshFirstP, USkeletalMeshComponent* SkCharWepMeshThirdP) { return true; }
void UFireMode::Server_Reliable_FireProjectileTrace_Implementation(APawn* Pawn, FHitResult hitResult, FVector CameraRotationXVec, USkeletalMeshComponent* SkCharWepMeshFirstP, USkeletalMeshComponent* SkCharWepMeshThirdP)
{
	...
	if (traceHitOut.Distance <= _fMaxRangeThreshold)
	{
		...
		if (traceHitOut.IsValidBlockingHit())
		{
			...
			{
				if (traceHitOut.GetActor()->CanBeDamaged())
				{
					...
					if (character != NULL)
					{
						...
						if (character->IsAlive())
						{
							...
							if (character->IsShieldDepleted())
							{
								...
								{
									if (_FCurveDamageOverDistanceMultiplier != NULL)
									{
										// Multiply damage against the curve (I normalize it to a range of [0f - 1f] as a precaution
										float multiplier = _FCurveDamageOverDistanceMultiplier->GetFloatValue(traceHitOut.Distance);
			...
			if (traceHitOut.GetComponent() != NULL)
			{
...
*
*/
bool UFireMode::OwningClient_Unreliable_DebugFireTrace_Validate(FVector StartPoint, FVector EndPoint) { return true; }
...
*
*/
bool UFireMode::Server_Reliable_FireProjectilePhysics_Validate(APawn* Pawn, FHitResult hitResult, FVector CameraRotationXVec, USkeletalMeshComponent* SkCharWepMeshFirstP, USkeletalMeshComponent* SkCharWepMeshThirdP) { return true; }
void UFireMode::Server_Reliable_FireProjectilePhysics_Implementation(APawn* Pawn, FHitResult hitResult, FVector CameraRotationXVec, USkeletalMeshComponent* SkCharWepMeshFirstP, USkeletalMeshComponent* SkCharWepMeshThirdP)
{
	...
	projectile->SetInstigator(_WeaponParentAttached->GetPawnOwner());
...
*
*/
bool UFireMode::OwningClient_SetFireDelayComplete_Validate(bool Complete) { return true; }
...
* @return:	void
*/
bool UFireMode::Server_Reliable_SetOverheated_Validate(bool Overheated) { return true; }
...
*
*/
bool UFireMode::Multicast_Unreliable_PlayThirdPersonMuzzle_Validate(USkeletalMeshComponent* SkCharWepMeshThirdP) { return true; }
...
*
*/
bool UFireMode::OwningClient_Unreliable_PlayFirstPersonMuzzle_Validate(USkeletalMeshComponent* SkCharWepMeshFirstP) { return true; }
...
*
*/
bool UFireMode::OwningClient_Reliable_RecoilCamera_Validate() { return true; }
...
*
*/
bool UFireMode::OwningClient_Reliable_StartRecoilInterpolation_Validate() { return true; }
...
*
*/
bool UFireMode::OwningClient_Reliable_StopRecoilInterpolation_Validate() { return true; }
...
*
*/
bool UFireMode::Server_Reliable_SetReloadComplete_Validate(bool ReloadComplete) { return true; }
void UFireMode::Server_Reliable_SetReloadComplete_Implementation(bool ReloadComplete) { SetReloadAnimationComplete(ReloadComplete); }
void UFireMode::SetReloadAnimationComplete(bool Complete)
{
	if (_WeaponParentAttached->GetLocalRole() == ROLE_Authority)
	{
		_bReloadComplete = Complete;

		ABaseCharacter* owningCharacter = Cast<ABaseCharacter>(_WeaponParentAttached->GetPawnOwner());
		if (owningCharacter)
		{
			if (_WeaponParentAttached->IsOwnersPrimaryWeapon())
			{
				owningCharacter->OwningClient_Reliable_PrimarySetCanFireWeapon(true);
				owningCharacter->SetIsReloadingPrimaryWeapon(false);
			}

			else if (_WeaponParentAttached->IsOwnersSecondaryWeapon())
			{
				owningCharacter->OwningClient_Reliable_SecondarySetCanFireWeapon(true);
				owningCharacter->SetIsReloadingSecondaryWeapon(false);
			}
		}
...
*
*/
bool UFireMode::Server_Reliable_EjectMagazine_Validate() { return true; }
...
*
*/
bool UFireMode::Server_Reliable_NewMagazine_Validate() { return true; }
...
*
*/
bool UFireMode::Server_Reliable_ChamberRound_Validate() { return true; }
...
*
*/
bool UFireMode::Server_Reliable_SetReloadStage_Validate(E_ReloadStage ReloadState) { return true; }
...
* @return:	void
*/
bool UFireMode::Server_Reliable_AddToSpread_Validate(float Additive, float ClampMinimum, float ClampMaximum) { return true; }
...
* @return:	void
*/
bool UFireMode::Server_Reliable_DeductFromSpread_Validate(float Subtractive, float ClampMinimum, float ClampMaximum) { return true; }
