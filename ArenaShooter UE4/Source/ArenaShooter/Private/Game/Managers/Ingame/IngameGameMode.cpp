...
void AIngameGameMode::InitialPlayerSpawn(ABasePlayerController* PlayerController)
{
	...
	if (!spawnLocation.Equals(FVector::ZeroVector))
	{
		...
		// Destroy old pawn & replace with new
...
void AIngameGameMode::OnPlayerSpawned(ABasePlayerController* PlayerController)
{
	AIngamePlayerState* playerState = Cast<AIngamePlayerState>(PlayerController->PlayerState);
	if (playerState == NULL) { return; }

	// If the player isn't a Spectator then they are an active player and are Alive
	if (playerState->GetAssignedTeam() != E_TeamNames::eTN_Spectator)
	{ playerState->Server_Reliable_SetSessionState(E_SessionState::eSS_Alive); }
	else
	{ playerState->Server_Reliable_SetSessionState(E_SessionState::eSS_Default); }
}

/*
*
*/
void AIngameGameMode::CollectActorGarbage(AActor* ActorToCollect, float Delay)
{
